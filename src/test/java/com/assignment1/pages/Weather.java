...
public class Weather {
	...
	public void scrollDown() {
		actions().click().sendKeys(Keys.PAGE_DOWN).build().perform();
	}
	
	public void validateWeatherCity(String city) {
		SelenideElement weatherCityName=$(By.xpath("//div[@class='outerContainer']//div[@class='cityText' and text()='"+city+"']"));
		//actions().click(weatherCityName).sendKeys(Keys.PAGE_DOWN).build().perform();
		Assert.assertEquals(city,weatherCityName.getText());
		Reporter.log("Expected : "+city+ "Actual : "+weatherCityName.getText(),true);
	}
	...
	SelenideElement leafletPlace=$(By.xpath("//div[@class='leaflet-popup-content']//span[contains(text(),'"+city+"')]"));
	Assert.assertTrue("Expected :"+city+ "******* Actual :" + leafletPlace.getText(),leafletPlace.getText().contains(city));
	}
	
	
	public String getWeatherCondition(String city) {
		SelenideElement cityWeatherLeaflet=$(By.xpath("//span[contains(text(),'"+city+"')]/ancestor::div[@class='leaflet-popup-content']"));
		if(cityWeatherLeaflet.isDisplayed()) {
			Assert.assertTrue("Condition Info not displayed",WeatherConditionInfo.shouldBe(Condition.visible).isDisplayed());
		}
	...
	public String getWeatherHumidity(String city) {
		SelenideElement cityWeatherLeaflet=$(By.xpath("//span[contains(text(),'"+city+"')]/ancestor::div[@class='leaflet-popup-content']"));
		if(cityWeatherLeaflet.isDisplayed()) {
			Assert.assertTrue("Humidity Info not displayed",WeatherHumidityInfo.shouldBe(Condition.visible).isDisplayed());
		}
		humid =WeatherHumidityInfo.getText().split(":");
		String[] humidvalue=humid[1].split("%");
		// int i = Integer.parseInt(humidvalue[0]);
		return humidvalue[0];		
	}
	...
	public double getWeatherTempDegree(String city) {
		SelenideElement cityWeatherLeaflet=$(By.xpath("//span[contains(text(),'"+city+"')]/ancestor::div[@class='leaflet-popup-content']"));
		if(cityWeatherLeaflet.isDisplayed()) {
			Assert.assertTrue("Humidity Info not displayed",WeatherTempInDegrees.shouldBe(Condition.visible).isDisplayed());
		}
	...
	public double getWeatherTempFahrenheit(String city) {
		SelenideElement cityWeatherLeaflet=$(By.xpath("//span[contains(text(),'"+city+"')]/ancestor::div[@class='leaflet-popup-content']"));
		if(cityWeatherLeaflet.isDisplayed()) {
			Assert.assertTrue("Humidity Info not displayed",WeatherTempInFahrenheit.shouldBe(Condition.visible).isDisplayed());
		}
